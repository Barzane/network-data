...
def adjacency_matrix(data, Nodes_dict, carrier):
    """FUNCTION	create an adjacency matrix for nodes in N
		INPUT	data dictionary data_yyyy_q.bin
        ...
        FOR	each item in the list of data_yyyy_q.bin keys
			...
			THEN
				...
				SET	element (column, row) of g = 1 <g[column][row] = 1>
				<There is no need for an error trap here, since the dictionary of 
    nodes is exhaustive, and is generated from data_2013_4.bin, i.e., 
    nodes[origin] and nodes[destination] will always be found>
		...
		THEN
			...
			WAIT	<use raw_input() to pause the code until a key is pressed; 
   is this a critical problem that requires the code to be terminated? if it is, 
   we could use import sys and sys.exit(0) to kill the code>
		ENDIF
