...
def density_degree_distribution(network):
    ...
    """FUNCTION	compute the density and the degree distribution of (N, g)
		INPUT	network tuple = (N, g), where N is a dictionary of nodes 
  (output from Task 5) and g is the corresponding adjacency matrix 
  (output from Task 6) <I think that it will be convenient to define 
  networks as (N, g), especially when we will have multiple quarters 
  and N or Nbar(s); a network can be defined as network = (N, g), 
  outside of the function>
    ...
    <in some Python implementations, e.g., 2/3 will give 0, so we force a float>
